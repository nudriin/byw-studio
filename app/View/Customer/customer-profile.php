            ...
            <div class="grid items-center justify-center grid-cols-12">
                ...
                <div class="col-span-12">
                    <div class="flex items-center justify-center">
                        <h1 class="mb-4 text-4xl font-rubik">Edit Profil</h1>
                    </div>
                    <form action="/customer/profile" method="post" class="col-span-6">
                        <div class="flex flex-col items-center justify-center gap-3">
                            <div>
                                <input type="text" name="username" id="" placeholder="Username" class="w-full p-2 border-2 rounded-lg border-abu bg-abu text-ungu" value="<?= $model['customer']->username ?>" disabled>
                            </div>
                            <div>
                                <input type="text" name="name" id="" placeholder="Name" class="w-full p-2 border-2 rounded-lg border-abu" required value="<?= $model['customer']->name ?? $model['post_name'] ?>">
                            </div>
                            <div>
                                <input type="number" min="0" name="phone" id="" placeholder="Phone" class="w-full p-2 border-2 rounded-lg border-abu" value="<?= $model['customer']->phone ?? $model['post_phone'] ?>">
                            </div>
                            <div>
                                <button type="submit" class="w-full p-2 bg-abu text-ungu font-rubik rounded-[15px]">Simpan</button>
                            </div>
                        </div>
                    </form>
                </div>
